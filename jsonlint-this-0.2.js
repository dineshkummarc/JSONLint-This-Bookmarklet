...
(function(document) {
	 ...
	 */
	function inspectorLog(msg, err) {
		// If console exists:
		if ('console' in window) {
			// If this was an error, and error method exists:
			if ( err && 'error' in window.console )
				// Error it:
				console.error(msg);
			else
				// If not, log it:
	...
	function inspectorOnClick(e) {
		...
		try {
			...
			if (result) {
				...
				e.target.innerText = JSON.stringify(result, null, "    ").replace(/\\\\/g, "\\");
			}
		} catch(err) {
			inspectorLog(err, 1);
		}
